...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=1000):
        ...
        # self.conv_u1 = Conv_Bn_Activation(256 + 512, 512, kernel_size=3, stride=1, activation='leaky')
        # self.conv_u2 = Conv_Bn_Activation(128 + 256, 128, kernel_size=3, stride=1, activation='leaky')

        self.mm_mrg2 = MRGblock(128+512, 512)
        self.mm_mrg3 = MRGblock(256 + 512, 512)
        self.mm_mrg4 = MRGblock(512, 512)

        self.linear_m2 = nn.Linear(512, num_classes)
        self.linear_s2 = nn.Linear(512, 1)

        self.linear_m3 = nn.Linear(512, num_classes)
        self.linear_s3 = nn.Linear(512, 1)

        self.linear_m4 = nn.Linear(512, num_classes)
        self.linear_s4 = nn.Linear(512, 1)

        self.linear_m5 = nn.Linear(512, num_classes)
        self.linear_s5 = nn.Linear(512, 1)
        # self.conv_b1 = Conv_Bn_Activation(128, 128, kernel_size=5, stride=4, activation='leaky')
        # self.conv_b2 = Conv_Bn_Activation(256, 256, kernel_size=3, stride=2, activation='leaky')
        # self.conv_b3 = Conv_Bn_Activation(512, 512, kernel_size=1, stride=1, activation='leaky')
        # self.conv_pu1 = nn.Conv2d(512+256+128, 512, kernel_size=1, padding=0)
...
#####################向上cat###########################
        ...
        out = m2 * score2 + m3 * score3 + m4 * score4 + m5 * score5
...
class MRGblock(nn.Module):
    def __init__(self, in_channel, mid_channel):
        ...
        self.conv_d0 = nn.Conv2d(mid_channel, mid_channel, kernel_size=3, dilation=3, padding=3, groups=mid_channel)
        self.conv_d1 = nn.Conv2d(mid_channel, mid_channel, kernel_size=3, dilation=5, padding=5, groups=mid_channel)
        self.conv_d3 = nn.Conv2d(mid_channel, mid_channel, kernel_size=3, dilation=7, padding=7, groups=mid_channel)
    ...
    def forward(self, x):
        x = self.conv_u(x)
        x_t1 = nonlinearity(self.conv_1x1(self.conv_d0(x)))
        x_t1 = self.ca1(x_t1)
        ...
        # out2 = self.out2(y)
        x_t3 = nonlinearity(self.conv_1x1(self.conv_d1(self.conv_1x1(self.conv_d0(x)))))
        x_t3 = self.ca3(x_t3)
        # x_t3 = self.conv_n3(x_t3)
        y3 = self.gru3(x_t3)

        # out3 = self.out3(y)
        x_t4 = nonlinearity(self.conv_1x1(self.conv_d3(self.conv_1x1(self.conv_d1(self.conv_1x1(self.conv_d0(x)))))))
        x_t4 = self.ca4(x_t4)
